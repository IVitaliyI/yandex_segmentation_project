...
load_from = 'mmsegmentation/practicum_work/artifacts/deeplabv3_baseline/epoch_50.pth'
log_level = 'INFO'
    ...
    decode_head=dict(
        ...
        loss_decode=[
            dict(
                ...
                type='CrossEntropyLoss',
                use_sigmoid=False),
            dict(loss_weight=3.0, type='DiceLoss', use_sigmoid=False),
        ],
        norm_cfg=dict(requires_grad=True, type='SyncBN'),
        num_classes=3,
        type='ASPPHead'),
...
optim_wrapper = dict(
    ...
    optimizer=dict(lr=0.0002, momentum=0.9, type='SGD', weight_decay=0.0005),
    type='OptimWrapper')
optimizer = dict(lr=0.0002, momentum=0.9, type='SGD', weight_decay=0.0005)
param_scheduler = [
    dict(
...
test_evaluator = dict(
    ...
    output_dir=
    'mmsegmentation/practicum_work/artifacts/results/deeplabv3_baseline/images',
    type='IoUMetric')
test_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(keep_ratio=False, scale=(
...
train_cfg = dict(max_epochs=15, type='EpochBasedTrainLoop')
...
work_dir = 'mmsegmentation/practicum_work/artifacts/deeplabv3_experiment1'
